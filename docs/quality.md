...
- FindingSuppression은 프로젝트 경계를 필수로 하며 코드 원문 대신 결정적인 SHA-256 지문만 저장한다.
- FindingSuppressionHit은 실행 시점의 제외 판단을 불변 스냅샷으로 보존하고 일반 Finding·보고서 집계와 분리한다.
- 만료 자동 삭제는 수동 프로젝트 삭제와 같은 DB·파일 경계 서비스를 재사용한다.
